...
When scoring **Argument Coherence**, **Methodological Rigor**, and **Evidence Sufficiency**, also check whether the paper deploys any of the 5 high-impact argumentation patterns documented in `references/argumentation_patterns.md` (patterns 1-5: counterintuitive thesis, multi-angle evidence convergence, sharp metric choice, scoped limitation, falsifiable prediction).
...
A paper exhibiting 2+ of these patterns is punching above its weight and the dimension scores above should reflect it. A paper exhibiting zero is likely safe but unmemorable — flag this in the weaknesses list with severity `MINOR`.
